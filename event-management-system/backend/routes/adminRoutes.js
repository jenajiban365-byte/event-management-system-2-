...
const { asyncRoute, todayIsoDate, COUNTED_BOOKING_STATUSES } = require('../utils/routeHelpers');
const router = express.Router();

router.get('/dashboard', protect, adminOnly, asyncRoute(async (req, res) => {
  ...
  const [totalEvents, upcomingEvents, totalUsers, totalBookings, activeBookings, totalClubs, pendingClubs, pendingEvents, recentBookingsRaw, trendRaw] = await Promise.all([
    ...
    Booking.countDocuments({ status: { $in: COUNTED_BOOKING_STATUSES } }),
...
}, 'Server error fetching dashboard.'));

router.get('/pending-events', protect, adminOnly, asyncRoute(async (req, res) => {
  const events = await Event.find({ status: 'pending_approval' }).populate('club', 'name').populate('organizer', 'name email').sort({ createdAt: -1 });
  res.json({ events });
}, 'Server error fetching pending events.'));

router.get('/clubs', protect, adminOnly, asyncRoute(async (req, res) => {
  const clubs = await Club.find().populate('createdBy', 'name email').populate('organizerIds', 'name email').sort({ createdAt: -1 });
  res.json({ clubs });
}, 'Server error fetching clubs.'));
